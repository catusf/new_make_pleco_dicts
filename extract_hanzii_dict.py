...
for num, filepath in enumerate(files):
    ...
    reccom_words_soup, word_detail_soup, hot_keywords_soup = list(content_result.next.children)
    ...
    chinese_word = (
        ""
        if not (chinese_word_soup := char_pron_soup.find("span", class_="simple-tradition-wrap"))
    ...
    hanzii_popularity = (
        str(BIGNUM)
        if not (hanzii_pop_soup := soup.find("div", class_="popularity-box"))
        else hanzii_pop_soup.find("span").text
    )
    ...
    # if amhanviet:
    #     pleco_string += f"{pleco_make_dark_gray(pleco_make_bold(PC_HANVIET_MARK))} {pleco_make_italic(amhanviet)}\n"

    # print(f'{chinese=} {pinyin=} {viet=}')

    wordkind_list = [item.text.strip() for item in word_detail_soup.find_all("span", class_="word-kind")]
    ...
    if not wordkinds_soups:
        single_def_soup = word_detail_soup.find("div", class_="detail-word content-result-white")
        if not single_def_soup:
            ...
            number = 1
            mean_chinese = single_def_soup.find("span", class_="simple-tradition-wrap").text.strip()
            mean_viet = single_def_soup.find("div", class_="txt-mean").text.strip()
            mean_chinese = remove_redundant_characters(remove_traditional_text(mean_chinese))
        ...
        for wordkind_soup in wordkinds_soups:
            ...
            # pleco_string += f"{pleco_make_dark_gray(pleco_make_bold(wordkind_text))}\n"
            # print(f'## {tuloai_text}')

            definitions_soup = wordkind_soup.find_all("div", class_="item-content")
            ...
            for num, definition_soup in enumerate(definitions_soup):
                ...
                number = definition_soup.find("div", class_="icon-dot").text.strip()[:-1]
                mean_viet = definition_soup.find("span", class_="simple-tradition-wrap").text

                mean_chinese = (
                    ""
                    if not (mean_chinese_soup := definition_soup.find("div", class_="txt-mean-explain"))
                ...
                # pleco_string += f"\n{pleco_make_dark_gray(pleco_make_bold(number_in_cirle(num+1)))} "
                # pleco_string += f"{pleco_make_blue(mean_chinese)} {mean_viet}\n"

                example_data = {"example": {}}

                if (example_soup := definition_soup.find("div", class_="box-example")) and example_soup.find(
                    "p", class_="ex-word"
                ):
                    example_chinese = example_soup.find("p", class_="ex-word").text

                    example_pron = example_soup.find("p", class_="ex-phonetic").text
                    example_meaning = example_soup.find("p", class_="ex-mean").text

                    example_chinese = remove_redundant_characters(remove_traditional_text(example_chinese))
                    example_pron = remove_redundant_characters(example_pron)
                    example_meaning = remove_redundant_characters(example_meaning)

                    # pleco_string += f"{pleco_make_dark_gray(PC_DIAMOND)} {pleco_make_dark_gray(PC_VIDU_NEW_MARK)}\n "
                    # pleco_string += (
                    #     f"{pleco_make_blue(example_chinese)} {pleco_make_italic(example_pron)} {example_meaning}\n"
                    # )
                ...
                dict_item["wordkinds"]["list_items"][wordkind_text].append(def_data)

    recommendations_soup = reccom_words_soup.find_all("div", class_="box-item")
    # pleco_string += f"\n{pleco_make_dark_gray(PC_CLUB_SUIT)} {pleco_make_dark_gray(PC_GOIY_MARK)}\n"

    dict_data[headword] = dict_item

    for recommendation_soup in recommendations_soup:
        reccomendation_mean = recommendation_soup.find("div", class_="box-mean").text

        reccomendation_chinese = recommendation_soup.find("span", class_="simple-tradition-wrap").text

        reccomendation_pinyin = (
            pinyinget(reccomendation_chinese)
            if not (rec_pinyin_s := recommendation_soup.find("div", class_="txt-pinyin"))
            else recommendation_soup.find("div", class_="txt-pinyin").text
        )

        reccomendation_chinese = remove_traditional_text(reccomendation_chinese)
        reccomendation_pinyin = remove_chinese_bracket(reccomendation_pinyin.lower())

        reccomendation_item = {
            "chinese": reccomendation_chinese,
        ...
        # pleco_string += f"{pleco_make_dark_gray(PC_ARROW)} {pleco_make_blue(reccomendation_chinese)} {pleco_make_italic(reccomendation_pinyin)} {reccomendation_mean}\n"

    # pleco_string = pleco_string.replace("\n\n", "\n")
    # print(pleco_string)
    # The above code is printing the value of the variable `pleco_string`.
    # pleco_string = pleco_string.replace("\n", PC_NEW_LINE)
